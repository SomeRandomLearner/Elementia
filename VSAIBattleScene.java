...
public class VSAIBattleScene extends JPanel {
    private static final Logger LOGGER = Logger.getLogger(VSAIBattleScene.class.getName());
    ...
    private final JPanel alliesPanel, enemiesPanel, skillsPanel;
    ...
    public VSAIBattleScene(Elementia frame) {
        ...
        try {
            URL leftUrl = getClass().getResource("/resources/LevelBackgrounds/Level" + level + "Background.png");
            if (leftUrl != null) {
                leftBgImage = ImageIO.read(leftUrl);
            } else {
                LOGGER.log(Level.WARNING, "Left background not found: {0}", "/resources/LevelBackgrounds/Level" + level + "Background.png");
            }

            URL rightUrl = getClass().getResource("/resources/LevelBackgrounds/Level" + level + "BackgroundMirror.png");
            if (rightUrl == null) {
                rightUrl = getClass().getResource("/resources/LevelBackgrounds/Level" + level + "BackgroundMirror.png");
            }
            if (rightUrl != null) {
                rightBgImage = ImageIO.read(rightUrl);
            } else {
                LOGGER.log(Level.WARNING, "Right background (mirror) not found for level {0}", level);
            }
        } catch (IOException ex) {
            LOGGER.log(Level.SEVERE, "Failed to load background images for level " + LevelSelectScene.selectedLevel, ex);
        }
        ...
        // Create center panel with overlay layout for result label
